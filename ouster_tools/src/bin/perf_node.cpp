...
{
public:
  PerfNode(const rclcpp::NodeOptions& opts)
    : Node("perf_node", opts),
      ...
      sample_count_(0)
  {
    ...
    if (this->mode_ == "jitter")
        ...
        if (this->topic_ == "/points")
          {
            this->points_sub_ =
              this->create_subscription<PCLMsg>(
                ...
                std::bind(&PerfNode::jitter_cb<PCLMsgPtr>, this, _1));
          }
        else
          {
      ...
      {
        RCLCPP_WARN(this->log_, "Unsupported 'mode': %s", this->mode_.c_str());
      }
  }
...
int main(int argc, char ** argv)
{
  ...
  auto node = std::make_shared<PerfNode>(options);
